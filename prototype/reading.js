...
learn=function(){return `<main class="container"><section class="curriculum-intro"><div class="eyebrow">一套课程 / 从第 1 课开始</div><h1>按顺序学，一课接着一课。</h1><p>六个单元只是把相近主题分组，共 18 课。点进每一课，就能在同页读讲解、看例子、检查理解，然后继续下一课。教材已放回课程内，不需要在两套目录之间切换。</p><div class="actions"><a class="button primary" data-go="lesson" href="${cLink(1)}">从第 1 课开始 →</a></div><p class="lesson-reading-note">第 1 课有互动模拟；第 2–18 课是入门阅读课，含例子与答案解释，尚无专属动画或训练实作。</p></section>${courseUnits.map(u=>`<section class="module-section"><div class="module-aside"><span class="num">单元 ${u.id}</span><h2>${u.title}</h2><p>${unitOutcomes[u.id-1]}</p></div><div>${courses.filter(c=>c.unit===u.id).map(c=>`<article class="course-row"><div class="course-row-top"><span class="number">${String(c.id).padStart(2,'0')}</span><h3><a href="${cLink(c.id)}" data-go="${coursePage(c.id)}">第 ${c.id} 课 · ${c.title}</a></h3><a class="course-open" href="${cLink(c.id)}" data-go="${coursePage(c.id)}" aria-label="打开第 ${c.id} 课">→</a></div><p>${c.question}</p><div class="course-meta"><span class="badge">${c.id===1?'互动实验 + 理解检查':'讲解 + 例子 + 理解检查'}</span></div></article>`).join('')}</div></section>`).join('')}</main>`};
...
render=function(){
  ...
  if(state.page==='lesson'){
    ...
    const next=$('.next-lesson');if(next){(main.querySelector('#quiz')||next).insertAdjacentHTML('afterend',coursePager(1));const materials=main.querySelector('.optional-reading');if(materials)next.insertAdjacentElement('afterend',materials)}
